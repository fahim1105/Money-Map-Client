...
const TransactionDetails = () => {
    ...
    return (
        ...
        <div className="min-h-screen bg-transparent flex items-center justify-center px-4 py-10">
            <div className="relative w-full max-w-6xl bg-base-100 backdrop-blur-2xl pt-15 rounded-3xl border border-primary-content shadow-2xl p-6 sm:p-10 overflow-hidden">

                {/* Back Button */}
                <button
                    onClick={() => navigate(-1)}
                    className="absolute left-4 top-4 flex items-center gap-2 text-sm text-neutral hover:text-primary transition"
                >
                    <FaArrowLeft /> Back
                </button>

                {/* Header */}
                <h2 className="text-4xl font-extrabold text-center mb-12 text-secondary">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    {/* LEFT */}
                    <div className="lg:col-span-2 bg-base-200 rounded-2xl p-6 border border-base-300 space-y-6">

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">

                            {/* Type */}
                            <div className="bg-primary/70 rounded-xl p-4 border border-base-300">
                                <p className="text-sm text-base-100 flex items-center gap-2">
                                    <FaMoneyBill /> Type
                                </p>
                                <p className={`text-xl font-semibold ${type === "Income" ? "text-green-500" : "text-red-500"
                                    }`}>
                                    {type}
                                </p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div className="bg-primary/70 rounded-xl p-5 border border-base-300">
                                <p className="text-sm text-base-100 flex items-center gap-2">
                                    <FaUser /> User
                                </p>
                                <p className="text-lg font-medium">{userName}</p>
                                <p className="text-sm text-neutral break-all">{email}</p>
                            </div>

                            <div className="bg-primary/70 rounded-xl p-5 border border-base-300 flex flex-col items-center justify-center">
                                <p className="text-sm text-base-100 mb-1">Status</p>
                                <span className={`px-4 py-1 rounded-full text-sm font-semibold
                                    ${status === "completed"
                                        ? "bg-green-600 text-primary-content"
                                        : "bg-gray-500 text-primary-content"
                                    }
              `}>
                                    {status}
                                </span>
                        ...
                        {/* Description */}
                        <div className="bg-primary/70 rounded-xl p-5 border border-base-300">
                            <p className="text-sm text-base-100 mb-2 flex items-center gap-2">
                    ...
                    {/* RIGHT */}
                    <div className="bg-base-200 rounded-2xl p-8 text-center border border-base-300 flex flex-col justify-center">
                        <h3 className="text-xl font-semibold text-neutral mb-2">
                            Total in this Category
                        </h3>
                        <p className="text-5xl font-extrabold text-base-100">
                            ${totalCategoryAmount.toFixed(2)}
                        </p>

                        <div className="mt-6 text-xs text-neutral break-all">
